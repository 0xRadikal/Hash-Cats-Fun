/*
 * verify_kernel.cu — prove the GPU keccak matches the contract, on the GPU box.
 *
 * Reads test_vectors.json (miner, prevWork, anchorHash, and a list of
 * nonce -> expected workHash produced by the live contract). For each nonce it
 * computes keccak256(packed(miner,nonce,prevWork,anchorHash)) on the GPU and
 * checks it equals the expected value. Prints PASS/FAIL per case.
 *
 * This MUST print ALL PASS before we trust the miner with real funds.
 *
 * Build: nvcc -O3 -arch=sm_80 verify_kernel.cu -o verify_kernel
 * Run:   ./verify_kernel test_vectors.json
 *
 * The device keccak here is #included from the same source as the miner to
 * guarantee they are identical.
 */
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
...
__device__ void keccak256_116(const uint8_t *msg, uint8_t out[32]) {
    uint64_t st[25];
...
__global__ void hash_one(const uint8_t *pre, uint8_t *out) {
...
/* ultra-minimal JSON field grabber for our known file shape */
...
int main(int argc, char **argv) {
    ...
    uint8_t *d_pre, *d_out;
    cudaMalloc(&d_pre, 116); cudaMalloc(&d_out, 32);

    int pass=0, fail=0;
    /* iterate over "nonce": N ... "workHash_hex": H pairs */
    const char *p = js;
    while ((p = strstr(p, "\"nonce\"")) != NULL) {
        const char *c = strchr(p, ':'); c++;
        unsigned long long nonce = strtoull(c, NULL, 10);
        char exp_hex[80];
        grab(p, "workHash_hex", exp_hex, sizeof(exp_hex));
        ...
        hash_one<<<1,1>>>(d_pre, d_out);
        cudaDeviceSynchronize();
        uint8_t got[32]; cudaMemcpy(got, d_out, 32, cudaMemcpyDeviceToHost);
    ...
    printf("\n%d PASS, %d FAIL -> %s\n", pass, fail,
           fail==0 ? "GPU KERNEL IS CORRECT" : "DO NOT USE — MISMATCH");
    free(js);
    return fail==0 ? 0 : 1;
}
